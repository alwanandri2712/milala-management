...
class Human_error extends CI_Controller
{
    var $site_url = 'Human_error';
    ...
    public function index()
    {
        ...
        $this->data['tittle']   = 'Human error';
        $this->data['tittle_2'] = 'Human error';
        $this->data['tittle_3'] = '';
        $this->data['content']  = 'human_error/index';
    ...
    public function add()
    {
        ...
        $insert = $this->Human_error_model->add($data);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->input->post('id')) {
            $update = $this->Human_error_model->edit($this->input->post('id'));
            if ($update) {
                $response = array(
                    'code'    => 200,
                    'status'  => 'success',
        ...
        $param['where']['id_human_error']  = $id;
        $getData = $this->Human_error_model->get_all(0, 1, 'id_human_error', 'desc', $param);
        if (!$getData['results']) {
            $response = array(
                'code'    => 404,
                'status'  => 'error',
    ...
    public function delete($id = null)
    {
        if ($this->session->userdata('id_role') == 1) { /* super administrator */
            $delete = $this->Human_error_model->delete($id);
            if ($delete) {
                $response = array(
                    'status'  => 'success',
...
/* End of file Human_error_model.php */
/* Location: ./application/controllers/Human_error_model.php */
